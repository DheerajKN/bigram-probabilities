...
def createBigram(data):
	...
	listOfBigrams = list(set(listOfBigrams))
...
if __name__ == '__main__':
	...
	inputExtended = sys.argv[1:]
	input = inputExtended[0]
	...
	output4 = open('Interpolation-OUTPUT.txt', 'w')
	outputProb1 = 1
	outputProb2 = 1
	outputProb3 = 1
	outputProb4 = 1

	inputList.append((input.split()[0],''))
	...
	output1.write(str(inputList[0]) + '\t\t' + str(unigramCounts[inputList[0][0]]) + '\t\t' + str(unigramCounts[inputList[0][0]]/len(data)) + '\n')
	outputProb1 = unigramCounts[inputList[0][0]]/len(data)
	...
	output2.write(str(inputList[0]) + '\t\t' + str(unigramCounts[inputList[0][0]]) + '\t\t' + str((unigramCounts[inputList[0][0]]+1)/(len(data)+len(unigramCounts))) + '\n')
	outputProb2 = (unigramCounts[inputList[0][0]]+1)/(len(data)+len(unigramCounts))

	for i in range(1,len(inputList)):
		if inputList[i] in bigramAddOne:
			output2.write(str(inputList[i]) + '\t\t' + str(bigramCounts[inputList[i]]) + '\t\t' + str(bigramAddOne[inputList[i]]) + '\n')
			...
			addOneCStar = 1 * unigramCounts[inputList[i][0]] / (len(data) + len(unigramCounts))
			outputProb2 *= prob
	...
	interpol = float(inputExtended[1])
	output4.write('Bigram\t\t\t\t' + 'Count\t\t\t\t' + 'Probability\n\n')
	output4.write(str(inputList[0]) + '\t\t' + str(unigramCounts[inputList[0][0]]) + '\t\t' + str(unigramCounts[inputList[0][0]]/len(data)) + '\n')
	outputProb4 = unigramCounts[inputList[0][0]]/len(data)

	for i in range(1,len(inputList)):
		if inputList[i] in bigramProb:
			val = (interpol * bigramProb[inputList[i]]) + ((1-interpol) * (unigramCounts[inputList[i][1]]/len(data)))
			output4.write(str(inputList[i]) + '\t\t' + str(bigramCounts[inputList[i]]) + '\t\t' + str(val) + '\n')
			outputProb4 *= val
		else:
			val = ((1-interpol) * (unigramCounts[inputList[i][1]]/len(data)))
			output4.write(str(inputList[i]) + '\t\t\t' + str(0) + '\t\t\t' + str(val) + '\n')
			outputProb4 *= val

	output4.write('\n' + 'Probablility = ' + str(outputProb4))
	print ('Interpolation: ', outputProb4)
